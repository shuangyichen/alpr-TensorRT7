...
def LP_detection(image):
    ...
    Llp,LlpImgs = reconstruct(Ivehicle,Iresized,trt_outputs,(240,80),lp_threshold)
    ...
    if len(LlpImgs):
        ...
        time_now = time.time()
        img_name = "%s%s.jpg" % (time.strftime("%Y%m%d%H%M%S_", time.localtime(time_now)), str(time_now % 1.0)[2:4])
            ...
            if (F==False):
                ...
                if LP==False:
                    ...
                    if majorvotefreq>=7:
                        ...
                        LP_str={'LP':LPstr}
                        time_now = time.time()
                        file_name = "%s%s.json" % (time.strftime("%Y%m%d%H%M%S_", time.localtime(time_now)), str(time_now % 1.0)[2:4])
...
parser.add_argument('-lpth' 	,'--lp-threshold'	,type=float   , default=.5		,help='lp_threshold')
parser.add_argument('-ocrth' 	,'--ocr-threshold'	,type=float   , default=.4		,help='ocr_threshold')
parser.add_argument('-onnx'		,'--onnx-file'		,type=str   , default='models/wpod0309b.onnx',help='path to onnx')
parser.add_argument('-e'		,'--engine'		    ,type=str   , default='model0309b.engine'	,help='path to TRT engine')
parser.add_argument('-output'	,'--output'	    	,type=str   , default='samples/output'		,help='output file path')
parser.add_argument('-ocrw'		,'--ocr-weights'	,type=str   , default='data/ocr/yolov3-LP_40000.weights'		,help='OCR weights')
parser.add_argument('-ocrcfg'	,'--ocr-netcfg'		,type=str   , default='data/ocr/yolov3-LP.cfg_train'	,help='OCR cfg file')
parser.add_argument('-ocrdata'	,'--ocr-dataset'	,type=str , default='data/ocr/LP.data'		,help='OCR dataset')
parser.add_argument('-a'		,'--avi'	,type=str , default='test0305.mp4'		,help='avi path')
